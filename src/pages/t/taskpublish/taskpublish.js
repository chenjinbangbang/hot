...
class Taskpublish extends React.Component {
    ...
    commentDelete = (index) => {
      ...
      this.setState({
        ...
        this.props.form.validateFields(['task_num'], {
          force: true
        })
      })
    }
    ...
    render() {
        ...
        return ( <
            ...
            金币 < /p> <
            p > 转发金币： {
              this.getTranspondAmount()
            }
            金币 < /p> <
            p className = "total" > 消耗金币总计： < span > {
              this.getTotalAmount()
            } < /span>金币</p >
            <
            p > （起步需0 .1 金币 / 任务） < /p> < /
            ...
            <
            p > 发布任务数： < span className = 'danger' > {
              this.state.form.task_num
            } < /span>个</p >
            <
            p > 消耗金币总计： < span className = 'danger' > {
              this.getTotalAmount()
            } < /span>金币</p >
            <
            p > 发布后不可修改任务， 请检查确定无误后再发布 < /p> < /
            ...
            fileUpload = {
              this.taskImageFileUpload.bind(this, 0)
            } > < /UploadImg> <
            Button className = 'img-example'
            type = 'link'
            onClick = {
              this.showImg.bind(this, taskImgs[form.getFieldValue('platform_type')].task_img1)
            } > 截图示例1 < /Button> <
            UploadImg isDetail = {
              false
            }
            ...
            fileUpload = {
              this.taskImageFileUpload.bind(this, 1)
            } > < /UploadImg> <
            Button className = 'img-example'
            type = 'link'
            onClick = {
              this.showImg.bind(this, taskImgs[form.getFieldValue('platform_type')].task_img2)
            } > 截图示例2 < /Button> <
            span className = 'taskpublish-note'
            style = {
              {
                marginTop: 'auto'
              }
            } > 上传任务截图， 让刷手更容易找到 < /span> < /
            div > <
            /Form.Item> <
            ...
            Form.Item label = '发布任务数' > {
              getFieldDecorator('task_num', {
                ...
                rules: [{
                  ...
                  validator: this.validateTaskNum
                }]
              })( <
                InputNumber min = {
                  1
            ...
            span className = 'taskpublish-gold' > 0.1 金币（ 默认关注） < /span> < /
            Form.Item > <
            Form.Item label = '关注保留时间'
            required > {
            ...
            <
            span className = 'taskpublish-gold'
            style = {
              {
                marginLeft: '0'
              }
            } > < span className = 'gold' > {
              form.getFieldValue('attention_time') * 10 * 0.1 / 10
            }
            金币 < /span></span >
            <
            br / >
            <
            span className = 'fontColor1' > 关注必须保留3个月， 创作者可在3个月内随机抽查刷手是否取消关注。 若3个月内查不到该刷手， 则扣除1金币给创作者， 并记录一次违规。 若选择了其他时间， 刷手在该时间内取消关注， 则在扣除n + 1 个金币， 如半年扣除2金币， 1 年扣除3金币， 2 年扣除4金币， 3 年扣除5金币， 并记录一次违规 < /span> < /
            Form.Item > <
            Form.Item label = '是否点赞'
            required > {
            ...
            span className = 'taskpublish-gold' > 0.1 金币（ 默认点赞） < /span> < /
            Form.Item > {
              // 今日头条才有收藏
              ...
              span className = 'taskpublish-gold' > 0.1 金币（ 默认收藏， 今日头条才有） < /span> < /
              Form.Item >
            }

            <
            Form.Item label = '是否评论'
            required > {
              ...
              form.getFieldValue('comment') &&
                <
                span className = 'taskpublish-gold' > 0.2 金币（ 默认评论） < /span>
            } <
            /Form.Item> {
            form.getFieldValue('comment') &&
            <
            ...
            ul > {
              comment_content.map((item, index) => ( <
                ...
                Icon type = "close"
                onClick = {
                  this.commentDelete.bind(this, index)
                }
                /> < /
                li >
              ))
        ...
        required > {
            ...
            form.getFieldValue('transpond') &&
              <
              span className = 'taskpublish-gold' > < span className = 'gold' > {
                this.getTranspondAmount()
              }
            金币 < /span>（0.2金币/转发数， < span className = {
                form.getFieldValue('task_num') < this.getTranspondAmount() * 10 / 0.2 / 10 ? 'danger' : null
              } > 注意： 转发数不能超过发布任务数 < /span>）</span >
        ...
        form.getFieldValue('transpond') &&
          ...
          div className = 'transpond-content' > {
            transpond_content.map((item, index) => ( <
                ...
                <
                div className = 'transpond-item'
                key = {
                  index
                } >
                <
                div className = 'transpond-type' > {
                  item.name
                }： < /div> {
              ...
              onChange = {
                this.changeTranspondNum.bind(this, index)
              } > < /InputNumber> <
              TextArea value = {
                item.content
              }
              ...
              onChange = {
                this.handleChangeTranspond.bind(this, index)
              }
              /> <
              div className = 'text-example' >
              <
              ...
              size = 'small'
              onClick = {
                this.transpondFn.bind(this, index)
              } > 确定 < /Button> < /
              div > <
              span className = 'taskpublish-gold' > {
                item.num * 10 * 0.2 / 10
                ...
                item.contentList.length > 0 &&
                  ...
                  ul > {
                    item.contentList.map((item1, index1) => ( <
                      ...
                      Icon type = "close"
                      onClick = {
                        this.transpondDelete.bind(this, index, index1)
                      }
                      /> < /
                      li >
                    ))
